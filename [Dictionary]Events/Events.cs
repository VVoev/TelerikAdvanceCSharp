namespace Events
{
    ...
    public class Events
    {
        ...
        private static void Main()
        {
            ...
            var info = new SortedDictionary<string, SortedDictionary<string, List<DateTime>>>();
            ...
            for (int i = 1; i <= N; i++)
            {
                ...
                if (match.Success && IsValidDate(match.Groups[3].Value))
                {
                    ...
                    {
                        info[location] = new SortedDictionary<string, List<DateTime>>();
                    }
                    if (!info[location].ContainsKey(person))
                    {
                        info[location][person] = new List<DateTime>();
                    }

                    info[location][person].Add(eventTime);
                }
            }
